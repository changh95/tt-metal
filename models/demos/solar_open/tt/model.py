...
def create_rope_setup(
    ...
    Solar-Open-100B uses YaRN (factor 2.0, original_max_position_embeddings 65536, attention factor
    0.1*ln(2)+1) on top of theta 1e6. On transformers 5.x the theta lives ONLY in
    ``hf_config.rope_parameters["rope_theta"]`` (there is no ``rope_theta`` attribute), so it is read through
    ``resolve_rope_theta``; ``rope_scaling_model_factory(hf_config.rope_scaling)`` (``rope_scaling`` is a property
    alias of ``rope_parameters``) yields the ``RopeScalingYarn`` that makes ``RotarySetup`` match
    ``SolarOpenRotaryEmbedding`` bit-for-bit. The cos/sin tables are ``[1, 1, max_position_embeddings, head_dim]``
    in two layouts (decode lookup + prefill), replicated on every device.

    Args:
        mesh_device: TTNN mesh device for computation
        hf_config: HuggingFace model configuration (``SolarOpenConfig``): ``rope_parameters``, ``head_dim``,
...
class Model:
    """
    Solar-Open TTNN Model Implementation

    This class implements upstage/Solar-Open-100B using TTNN tensors and operations.
    It supports both prefill and decode modes; every layer is full (non-sliding) GQA attention.

    Key Features:
    - MoE (Mixture of Experts) architecture: sigmoid router with selection bias, 128 routed experts (top-8)
      plus one always-on shared expert, one TP all-reduce per layer
    - YaRN RoPE (theta 1e6, factor 2.0) through tt_transformers' RotarySetup
    - Paged attention support for memory efficiency
    - Untied lm_head padded to a per-device power-of-two width so on-device sampling can run at TP=8
    ...
    def __init__(
        ...
        Args:
            mesh_device: TTNN mesh device for computation
            hf_config: HuggingFace model configuration (``SolarOpenConfig``)
            state_dict: Model weights dictionary in the transformers-5 layout (``{}`` = load every tensor from
                the ttnn cache at ``tensor_cache_path``)
            ccl_manager: Collective communication manager
            dtype: Data type for attention / lm_head weights (default: bfloat16)
            ...
            create_kv_cache: Whether every layer allocates its own KV cache (vLLM passes False)
            max_local_batch_size: Decode users per mesh row (also the router/experts' ``tokens_per_device``)
            users_row_sharded: Whether users are sharded across mesh rows (multi-row meshes only)
            moe_options: ``config.MoEOptions`` (expert dtypes, router implementation); None = Solar defaults
            max_seq_len: Sequence length the UNPAGED KV cache is sized for; None falls back to
                ``hf_config.max_position_embeddings``. Paged runs ignore it.
        """
        ...
        # max_seq_len only sizes the unpaged KV cache; without an explicit value fall back to the model's
        # context length rather than DecoderLayer's 1024 default, which would silently truncate long runs.
    ...
    def create_transformer_compatible(
        ...
        """Constructor compatible with the tt_transformers.Transformer interface.

        ``moe_options`` defaults to ``args.moe_options`` (``ModelArgs`` reads it from the SOLAR_OPEN_* env
        vars) and ``max_seq_len`` is taken from ``args.max_seq_len`` so the unpaged KV cache matches the
        demo's context length.
        """
        # Create the CCL manager (semaphores + ping-pong buffers) for the mesh
    ...
    def _forward_layers_and_head(
        ...
        Args:
            ...
            page_tables_per_layer: Optional list of per-layer page tables, one
                ...
                full-attention layers (KV cache groups). Solar-Open has uniform
                full attention, so vLLM normally passes None; the plumbing is
                kept as a harmless pass-through. When None, behavior is
    ...
    def ttnn_decode_forward(
        ...
        input_embeds = ttnn.embedding(
            tokens_for_embed, self.embedding_weight, layout=ttnn.TILE_LAYOUT, dtype=ttnn.bfloat16
        )
    ...
    def process_logits_after_prefill_trace(self, logits, last_token_idx):
        ...
        Unlike tt_transformers `Transformer`, Solar-Open's `ttnn_prefill_forward` already
    ...
    def row_sharded_batched_prefill(
        ...
        if (max(all_last_idxs) // 32) * 32 != fixed_glt:
            fixed_glt = -1
        # Solar-Open's first generated token is data dependent (usually <|think|> or <|content|> depending on
        # reasoning_effort), so lm_head always runs; sampling_params only selects greedy token output below.
        skip_lm = False

        vocab_size = model_args.vocab_size if model_args else self.vocab_size
        output_tensor = torch.zeros(batch_size, 1, vocab_size)
        trace_key = "rsbp_" + str(max_padded_len) + "_upr" + str(upr) + "_lm"
        ...
        if sampling_params is not None:
            # Pre-sampled first token per user (vLLM device sampling accepts token ids): greedy argmax over
            # the full-vocab logits, with zero log-probs. out is [out_n, 1, vocab] -> both outputs [out_n, 1].
            tokens = out.argmax(-1).to(torch.int64)
            return tokens, torch.zeros(out.shape[0], 1, dtype=torch.float32)
