        ...
        if dense_moe:
            ...
            if bmm_config is None:
                plan = _sorted_moe_plan(
                    routing_tokens_all, token_offset, split_len, config, program_config.dense_bmm_max_tokens
                )
            if plan is not None:
                ...
                next_states_reduced = _dense_tail(
                    gate_up, routing_split, split_len, weights, config, activation_dtype, dense_core_grid, ip
                )
        else:
...
def _concat_expert_slices(weight, expert_ids, rows, cols):
    """[1, len(expert_ids), rows, cols] on-demand copy of the listed experts of a [1, E, rows, cols] weight.
...
def _dense_tail(gate_up, routing_split, split_len, weights, config, activation_dtype, dense_core_grid, ip):
    ...
    linear) and the per-expert outputs are reduced over the expert dimension."""
...
# Cost model (ms per 1024-token split, P150) used to pick the hot/cold threshold on the host. Measured for the
# H=2880 / Ip=384 shapes this module was tuned on (tt-metal PR #55589); re-measure for H=4096 / Ip=160. The
# constants only steer the hot/cold split and the sorted-vs-dense choice, never correctness.
...
def _sorted_moe_plan(routing_tokens_all, token_offset, split_len, config, dense_bmm_max_tokens):
...
def _sorted_moe_forward(hidden_4D, plan, split_len, weights, config, activation_dtype, dense_core_grid):
    ...
    matmuls over the whole split (weight slices taken on demand and freed), weighted by their routing weights and
    reduced. The math equals the dense path."""
    ...
    if hot_ids:
        ...
        act_h = apply_glu(gate_h, up_h, config)
        ...
        wd_hot = _concat_expert_slices(
            weights.down_proj_padded, hot_ids, weights.down_proj_padded.shape[2], weights.down_proj_padded.shape[3]
        )
        down_h = ttnn.matmul(
            act_h,
            wd_hot,
