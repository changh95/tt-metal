...
"""Shared test scaffolding for the Solar-Open-100B demo: mesh/fabric parametrisation, ModelArgs/MeshConfig/CCL
setup and PCC comparison helpers (contract C12 of the design)."""
...
# Key of the models/model_trace_region_sizes.yaml block that sizes the trace region for this model.
TRACE_MODEL_KEY = "solar-open-100b"


def get_num_devices():
    """Number of devices the mesh parametrizations may use.

    ``ttnn.get_num_devices()`` enumerates the PCIe cluster, which happens at import / collection time because the
    mesh decorators build their parameter lists when the test module is imported. ``SOLAR_OPEN_NUM_DEVICES=<n>``
    overrides it so test modules can be imported and collected (``pytest --collect-only``) on a host without
    touching the devices, e.g. while another process owns them.
    """
    override = os.getenv("SOLAR_OPEN_NUM_DEVICES")
...
class TestFactory:
    ...
    def setup_test(mesh_device, use_real_weights=True, dtype=ttnn.bfloat8_b):
        ...
        ``HF_MODEL`` must resolve to a directory (or repo id) whose basename is ``Solar-Open-100B`` and that
        holds a ``config.json`` even for random-weight tests (``models/demos/solar_open/configs/Solar-Open-100B``
        works when no checkpoint is present); the tokenizer is only loaded with ``use_real_weights=True``.
        """
        ...
        from models.demos.solar_open.config import ModeConfig
...
def parametrize_mesh_with_fabric(mesh_shapes=None):
    """Universal mesh + fabric parametrization for Solar-Open tests.
    ...
    around). Multi-device shapes use ``FABRIC_1D_RING`` — the CCL
    operations of this model (reduce_scatter, all_gather, all_reduce) all
    use the ring topology.

    Trace region: ``TRACE_MODEL_KEY_PARAM`` defers the ``trace_region_size``
    to the ``mesh_device`` fixture, which resolves the ``solar-open-100b``
    block of ``models/model_trace_region_sizes.yaml`` for the SKU actually
    opened. ``SOLAR_OPEN_TRACE_REGION_SIZE=<bytes>`` overrides it (``0`` =
    dynamic allocation). ``SOLAR_OPEN_NUM_DEVICES=<n>`` replaces the device
    enumeration (host-only collection, see ``get_num_devices``).
    ...
    if not mesh_shapes:
        params = [
            pytest.param(
                (1, 1),
                {"fabric_config": None, TRACE_MODEL_KEY_PARAM: TRACE_MODEL_KEY},
                id="1x1",
                marks=pytest.mark.skip(reason="No supported Solar-Open mesh shape fits on this system"),
            )
        ]
    else:
        # SOLAR_OPEN_TRACE_REGION_SIZE (bytes) overrides the YAML-resolved trace region size (debug knob).
        trace_override = os.getenv("SOLAR_OPEN_TRACE_REGION_SIZE")
        trace_param = (
            {"trace_region_size": int(trace_override)} if trace_override else {TRACE_MODEL_KEY_PARAM: TRACE_MODEL_KEY}
        )
        params = [
            pytest.param(
                shape,
