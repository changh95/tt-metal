...
models/demos/solar_open/tt/experts/prefill.py (_sorted_moe_plan / _sorted_moe_forward) and compares against the
transformers reference (SolarOpenNaiveMoe, routed experts only: shared_expert=None). The uniform random routing of
the experts unit test never produces hot experts, so the hot/cold partition and its cost-model cap selection are
only covered here."""
...
def test_experts_prefill_skewed_routing(mesh_device, device_params, seq_len):
    ...
    if config.num_local_experts < experts_prefill._SORTED_MOE_MIN_EXPERTS:
        pytest.skip("the expert-sorted prefill path is only used for models with >= 64 experts (Solar-Open: 128)")
    ...
    with torch.no_grad():
        # SolarOpenNaiveMoe.forward(hidden_states, top_k_index, top_k_weights) -- positional
        reference = reference_layer.mlp.experts.eval()(
            hidden.reshape(-1, config.hidden_size), router_indices, routing_by_position
        )
