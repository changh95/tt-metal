...
"""
Unit tests for the Solar-Open RoPE (Rotary Position Embedding) implementation.

This module compares our rope setup (sin/cos embedding matrices) with the HuggingFace transformers reference
implementation (``SolarOpenRotaryEmbedding``). Solar-Open-100B uses YaRN scaling (factor 2.0 beyond
``original_max_position_embeddings`` = 65536, attention factor 0.1 * ln(2) + 1 = 1.0693) on head_dim 128 with
rope_theta 1e6. transformers 5.x stores the theta only in ``hf_config.rope_parameters``, so every table here is built
through ``resolve_rope_theta`` - the old ``getattr(hf_config, "rope_theta", <default>)`` silently returned the default.

``HF_MODEL`` selects the config; when unset the bundled ``configs/Solar-Open-100B`` is used (no weights are needed).
"""
...
from models.tt_transformers.tt.common import RopeScalingYarn, rope_scaling_model_factory
from models.tt_transformers.tt.rope import YarnRotaryEmbedding, compute_gather_cos_sin, rotary_embedding_factory

from ..test_factory import parametrize_mesh_with_fabric

# Config used when HF_MODEL is unset: the bundled Solar-Open-100B config.json (no checkpoint needed for RoPE tests).
_DEFAULT_HF_MODEL = "models/demos/solar_open/configs/Solar-Open-100B"

# YaRN attention factor for Solar-Open (factor 2.0, mscale defaults): 0.1 * ln(2) + 1 = 1.0693147. HF and the TT
# tables both multiply cos AND sin by it, so |cos|, |sin| reach 1.0693 and the rotary kernel must handle values > 1.
_SOLAR_YARN_ATTENTION_FACTOR = 0.1 * math.log(2.0) + 1.0

# bf16 tables hold one rounding of the fp32 value (ulp 2^-7 for |v| in [1, 2)); allow ~2 ulps against an fp32 reference.
_ROPE_BF16_LOOKUP_ATOL = 0.02
...
def _hf_model_path():
    """HF_MODEL if set, else the bundled Solar-Open-100B config directory."""
    return os.getenv("HF_MODEL") or _DEFAULT_HF_MODEL


def _load_hf_config():
    """SolarOpenConfig for the model under test (config only; no weights are touched)."""
    return AutoConfig.from_pretrained(_hf_model_path(), trust_remote_code=True)


def _hf_rope_at_positions(hf_config, position_ids, dtype=torch.float32):
    """Raw HF cos/sin ([1, P, head_dim], rotate_half layout, attention factor included) at explicit positions.

    SolarOpenRotaryEmbedding only reads ``x.dtype`` / ``x.device`` from its first argument, so a 1-element tensor
    selects the output dtype without materialising a [1, P, hidden] activation.
    """
    rope_hf = SolarOpenRotaryEmbedding(hf_config)
    x = torch.zeros(1, dtype=dtype)
    return rope_hf(x, position_ids)


def _hf_cos_sin_unique_at_positions(hf_config, positions, dtype=torch.float32):
    """HF cos/sin at ``positions`` reduced to the unique half: ([P, head_dim // 2], [P, head_dim // 2])."""
    position_ids = torch.tensor(positions, dtype=torch.long).unsqueeze(0)
    cos_hf, sin_hf = _hf_rope_at_positions(hf_config, position_ids, dtype)
    return extract_unique_from_hf_format(cos_hf, sin_hf)


def _yarn_check_positions(max_position_embeddings, original_max_position_embeddings):
    """Positions for the YaRN gate: the first 100, a few round numbers, both sides of the interpolation boundary
    (original_max_position_embeddings) and the last 128 positions of the table."""
    orig = original_max_position_embeddings
    positions = (
        list(range(100))
        + [4095, orig - 1, orig, orig + 1, 100000]
        + list(range(max_position_embeddings - 128, max_position_embeddings))
    )
    return sorted({p for p in positions if 0 <= p < max_position_embeddings})


def _device_lookup_positions(max_position_embeddings, original_max_position_embeddings, batch_size=32):
    """``batch_size`` distinct positions: both sides of the YaRN boundary, the table ends, and an even spread."""
    orig, max_pos = original_max_position_embeddings, max_position_embeddings
    boundary = [0, 1, orig - 2, orig - 1, orig, orig + 1, orig + 2, orig + 64, max_pos - 2, max_pos - 1]
    positions = sorted({p for p in boundary if 0 <= p < max_pos})
    spread = torch.linspace(2, max_pos - 3, batch_size - len(positions)).round().long().tolist()
    positions.extend(p for p in spread if p not in positions)
    filler = 3
    while len(positions) < batch_size:  # only if a spread point collided with the boundary set
        if filler not in positions:
            positions.append(filler)
        filler += 1
    positions = sorted(positions)[:batch_size]
    assert len(positions) == batch_size, positions
    return positions
...
def get_rope_cos_sin_from_hf_reference(hf_config, seq_len, dtype=torch.float32):
    """
    Get cos/sin matrices from the HuggingFace transformers reference implementation.

    This uses the official HuggingFace modeling_solar_open.SolarOpenRotaryEmbedding class as the reference
    (YaRN-scaled, attention factor applied to both cos and sin).

    Args:
        hf_config: HuggingFace config
        seq_len: Sequence length (positions 0 .. seq_len - 1)
        dtype: Output dtype (HF casts cos/sin to the activation dtype; default fp32)

    Returns:
        Tuple of (cos_ref, sin_ref) tensors in HF format [1, seq_len, head_dim] (see extract_unique_from_hf_format)
    """
    position_ids = torch.arange(seq_len, dtype=torch.long).unsqueeze(0)
    return _hf_rope_at_positions(hf_config, position_ids, dtype)
...
def get_rope_cos_sin_from_our_pytorch_impl(hf_config, seq_len):
    ...
    rotary_emb = rotary_embedding_factory(
        ...
        base=resolve_rope_theta(hf_config),
...
def extract_unique_from_hf_format(cos_hf, sin_hf):
    ...
    SolarOpenRotaryEmbedding returns cos/sin of shape [batch, seq_len, head_dim] built as cat(freqs, freqs) (the
    rotate_half convention): the second half of the last dimension duplicates the first, so the unique values are
    [..., :head_dim // 2] = (cos0, cos1, cos2, ...).

    Args:
        cos_hf: Cosine tensor in HF format [1, seq_len, head_dim]
        sin_hf: Sine tensor in HF format [1, seq_len, head_dim]

    Returns:
        Tuple of (cos_unique, sin_unique) tensors with shape [seq_len, head_dim // 2]
    """
    half = cos_hf.shape[-1] // 2
    cos_unique = cos_hf.squeeze(0)[..., :half]  # [seq_len, head_dim // 2]
    sin_unique = sin_hf.squeeze(0)[..., :half]  # [seq_len, head_dim // 2]
...
def test_rope_vs_hf_reference(mesh_device, device_params, seq_len, max_local_batch_size, reset_seeds):
    ...
    logger.info(f"Model config: {_hf_model_path()}")
    logger.info(f"Rope scaling config: {hf_config.rope_scaling}")
    logger.info(f"Rope theta: {resolve_rope_theta(hf_config)}")
...
@pytest.mark.parametrize(
    ...
    [
        (32, 1.0),  # Plain RoPE: cos/sin in [-1, 1]
        (32, _SOLAR_YARN_ATTENTION_FACTOR),  # Solar-Open YaRN: cos/sin in [-1.0693, 1.0693] - must pass
    ],
    ids=["scale_1.0_normal", "scale_1.0693_solar_yarn"],
)
def test_rotary_embedding_llama_kernel_with_scaled_values(
    mesh_device, device_params, batch_size, cos_sin_scale, reset_seeds
):
    """
    Direct test of the rotary_embedding_llama decode kernel with YaRN-scaled cos/sin at head_dim 128.

    YaRN multiplies cos and sin by the attention factor, so the bf16 cos/sin tables carry values with |v| > 1
    (1.0693 for Solar-Open). This test invokes ttnn.experimental.rotary_embedding_llama on the decode layout
    (one user per core, 32 padded heads x head_dim per shard) and compares against a PyTorch reference:

    - cos_sin_scale=1.0: plain RoPE, every batch position must pass PCC > 0.99
    - cos_sin_scale=1.0693 (Solar-Open YaRN): every batch position must pass as well
    - larger scales (not used by Solar-Open) only document the kernel's behaviour (xfail on failure)

    Args:
        mesh_device: TTNN mesh device fixture
        batch_size: Number of users in batch
        cos_sin_scale: Scale factor applied to cos/sin (1.0 = plain RoPE, 1.0693 = Solar-Open YaRN)
        reset_seeds: Reset random seeds fixture
    """
    ...
    head_dim = 128  # Solar-Open head dimension
    ...
    # Apply YaRN-style scaling (the attention factor multiplies both cos and sin)
    ...
    # Assertions: plain RoPE and Solar-Open's YaRN magnitude (1.0693) must both pass; larger scales (unused by
    # Solar-Open) only document the kernel's behaviour with |cos|, |sin| well above 1.
    if cos_sin_scale <= _SOLAR_YARN_ATTENTION_FACTOR + 1e-6:
        assert len(failed_positions) == 0, (
            f"rotary_embedding_llama failed for {len(failed_positions)} batch positions with cos/sin scale "
            f"{cos_sin_scale:.4f}. Failed positions: {failed_positions}. Min PCC: {min(pcc_results):.4f}"
        )
        logger.info(f"✓ All batch positions passed with cos/sin scale {cos_sin_scale:.4f}")
    elif failed_positions:
        logger.warning(
            f"{len(failed_positions)}/{batch_size} batch positions failed with cos/sin scale {cos_sin_scale:.4f}"
        )
        pytest.xfail(
            f"rotary_embedding_llama fails for {len(failed_positions)} batch positions when cos/sin values reach "
            f"{cos_sin_scale:.4f} (beyond Solar-Open's YaRN range). Min PCC: {min(pcc_results):.4f}"
        )
    else:
        logger.info(f"✓ All batch positions passed with cos/sin scale {cos_sin_scale:.4f}")
...
def test_rope_yarn_values_match_hf(mesh_device, device_params, reset_seeds):
    """
    YaRN gate (host, fp32): the cos/sin tables built exactly as RotarySetup builds them
    (compute_gather_cos_sin with the parsed RopeScalingYarn and resolve_rope_theta) must match
    SolarOpenRotaryEmbedding to < 1e-5 at positions on both sides of original_max_position_embeddings (65536)
    and up to the end of the max_position_embeddings (131072) table, and the tables must really be YaRN-scaled
    (carry the 1.0693 attention factor and differ from plain RoPE beyond the boundary).
    """
    hf_config = _load_hf_config()
    head_dim = hf_config.head_dim
    max_pos = hf_config.max_position_embeddings
    theta = resolve_rope_theta(hf_config)
    rope_scaling = rope_scaling_model_factory(hf_config.rope_scaling)

    # transformers 5.x keeps the theta only in rope_parameters; resolve_rope_theta must pick it up (not a default).
    assert theta == float(hf_config.rope_parameters["rope_theta"]), (theta, hf_config.rope_parameters)
    assert isinstance(rope_scaling, RopeScalingYarn), f"Solar-Open uses YaRN scaling, parsed {rope_scaling}"
    orig = rope_scaling.original_max_position_embeddings
    positions = _yarn_check_positions(max_pos, orig)

    logger.info("=== YaRN cos/sin table comparison ===")
    logger.info(f"theta={theta}, yarn={rope_scaling.model_dump()}, head_dim={head_dim}")
    logger.info(f"{len(positions)} positions: {positions[:3]} ... {orig - 1}, {orig}, {orig + 1} ... {positions[-1]}")

    # Ours: the RotarySetup call path (models/tt_transformers/tt/rope.py get_rot_mats -> compute_gather_cos_sin)
    cos_ours, sin_ours = compute_gather_cos_sin(dhead=head_dim, end=2 * max_pos, theta=theta, rope_scaling=rope_scaling)
    assert cos_ours.dtype == torch.float32 and tuple(cos_ours.shape) == (1, 1, max_pos, head_dim), cos_ours.shape
    cos_ours_unique, sin_ours_unique = extract_unique_from_meta_format(cos_ours, sin_ours)  # [max_pos, head_dim // 2]
    cos_ours_unique, sin_ours_unique = cos_ours_unique[positions], sin_ours_unique[positions]

    # HF reference at the same positions (fp32)
    cos_hf_unique, sin_hf_unique = _hf_cos_sin_unique_at_positions(hf_config, positions)
    assert cos_hf_unique.shape == cos_ours_unique.shape, (cos_hf_unique.shape, cos_ours_unique.shape)

    worst = 0.0
    for name, ref, ours in (("cos", cos_hf_unique, cos_ours_unique), ("sin", sin_hf_unique, sin_ours_unique)):
        diff = (ref - ours).abs()
        row, col = torch.unravel_index(diff.argmax(), diff.shape)
        logger.info(f"{name}: max |diff| {diff.max().item():.3e} at position {positions[row]} dim {col.item()}")
        worst = max(worst, diff.max().item())
    assert worst < 1e-5, f"YaRN cos/sin tables do not match HF SolarOpenRotaryEmbedding (max |diff| {worst:.3e})"

    # The attention factor 0.1 * ln(factor) + 1 (1.0693 for Solar-Open) multiplies cos and sin on both sides:
    # cos(0) * factor sits at position 0.
    expected_factor = YarnRotaryEmbedding.yarn_get_mscale(
        rope_scaling.factor, rope_scaling.mscale
    ) / YarnRotaryEmbedding.yarn_get_mscale(rope_scaling.factor, rope_scaling.mscale_all_dim)
    logger.info(f"attention factor: expected {expected_factor:.7f}, table {cos_ours[0, 0, 0, 0].item():.7f}")
    assert abs(cos_ours[0, 0, 0, 0].item() - expected_factor) < 1e-6
    assert abs(cos_hf_unique[positions.index(0), 0].item() - expected_factor) < 1e-6

    # ... and YaRN really changes the frequencies used beyond the boundary (guards against a rope_scaling=None or
    # unscaled-table regression that a low-position-only comparison would not catch).
    cos_plain, _ = compute_gather_cos_sin(dhead=head_dim, end=2 * max_pos, theta=theta, rope_scaling=None)
    beyond = [p for p in positions if p > orig]
    plain_diff = (cos_plain[0, 0, beyond, ::2] - cos_ours[0, 0, beyond, ::2]).abs().max().item()
    assert plain_diff > 0.1, f"tables beyond position {orig} equal unscaled RoPE (max diff {plain_diff:.3e})"

    logger.info("✓ Our YaRN cos/sin tables match HuggingFace at every checked position!")


@parametrize_mesh_with_fabric()
def test_rope_yarn_device_lookup_matches_hf(mesh_device, device_params, reset_seeds):
    """
    YaRN gate (device, bf16): RotarySetup.get_rot_mats(positions) - the decode-time embedding lookup on the bf16
    cos/sin tables that create_rope_setup builds (theta and YaRN parameters included) - must match
    SolarOpenRotaryEmbedding at positions on both sides of original_max_position_embeddings and at the end of the
    table, on every device of the mesh.
    """
    hf_config = _load_hf_config()
    rope_scaling = rope_scaling_model_factory(hf_config.rope_scaling)
    assert isinstance(rope_scaling, RopeScalingYarn), f"Solar-Open uses YaRN scaling, parsed {rope_scaling}"
    ...
    batch_size = 32
    positions = _device_lookup_positions(
        hf_config.max_position_embeddings, rope_scaling.original_max_position_embeddings, batch_size
    )
    logger.info(f"=== YaRN device lookup at positions {positions} ===")
    ...
    cos_tt, sin_tt = rope_setup.get_rot_mats(torch.tensor(positions, dtype=torch.int64))

    # fp32 reference [batch, head_dim // 2]; the device tables are bf16 (one rounding of the same fp32 values)
    cos_hf_unique, sin_hf_unique = _hf_cos_sin_unique_at_positions(hf_config, positions)

    cos_devices = ttnn.get_device_tensors(cos_tt)
    sin_devices = ttnn.get_device_tensors(sin_tt)
    for dev_idx, (cos_dev, sin_dev) in enumerate(zip(cos_devices, sin_devices)):
        cos_dev_torch = ttnn.to_torch(cos_dev).float()  # [1, batch, 32 (1 padded to a tile), head_dim]
        sin_dev_torch = ttnn.to_torch(sin_dev).float()
        assert cos_dev_torch.shape[1] >= batch_size, cos_dev_torch.shape

        for user_idx, pos in enumerate(positions):
            cos_user = cos_dev_torch[0, user_idx, 0, ::2]  # Meta format -> unique values [head_dim // 2]
            sin_user = sin_dev_torch[0, user_idx, 0, ::2]
            cos_ok, cos_pcc = _compare_rope_vector(cos_hf_unique[user_idx], cos_user, 0.99)
            sin_ok, sin_pcc = _compare_rope_vector(sin_hf_unique[user_idx], sin_user, 0.99)
            cos_err = (cos_hf_unique[user_idx] - cos_user).abs().max().item()
            sin_err = (sin_hf_unique[user_idx] - sin_user).abs().max().item()
            if dev_idx == 0:
                logger.info(
                    f"position {pos}: cos PCC {cos_pcc:.6f} max|err| {cos_err:.4f}, "
                    f"sin PCC {sin_pcc:.6f} max|err| {sin_err:.4f}"
                )
            assert cos_ok and cos_err < _ROPE_BF16_LOOKUP_ATOL, (
                f"device {dev_idx}, user {user_idx}, position {pos}: cos mismatch "
                f"(PCC {cos_pcc:.6f}, max |err| {cos_err:.4f})"
            )
            assert sin_ok and sin_err < _ROPE_BF16_LOOKUP_ATOL, (
                f"device {dev_idx}, user {user_idx}, position {pos}: sin mismatch "
                f"(PCC {sin_pcc:.6f}, max |err| {sin_err:.4f})"
            )

    logger.info(
        f"✓ Device RoPE lookup matches HuggingFace at all {batch_size} positions on {len(cos_devices)} devices!"
    )
...
def test_trace_rope_ops_for_corruption(mesh_device, device_params, reset_seeds):
    ...
    head_dim = hf_config.head_dim  # 128
...
def test_paged_update_cache_with_large_values(mesh_device, device_params, reset_seeds):
    ...
    # Create K tensor with large values (simulating YaRN-scaled Q/K). Solar-Open's attention factor is 1.0693;
    # 1.35 is kept as a stress margin above it.
    torch.manual_seed(42)
    k_torch_normal = torch.randn(1, batch_per_row, num_kv_heads, head_dim, dtype=torch.bfloat16)
    k_torch_large = k_torch_normal * 1.35  # Simulate YaRN scaling (stress margin above 1.0693)
...
def test_sdpa_with_large_q_values(mesh_device, device_params, reset_seeds):
    ...
    q_torch_large = q_torch_normal * 1.35  # YaRN-like scaling (stress margin above Solar-Open's 1.0693)
...
def test_single_layer_with_yarn(mesh_device, device_params, reset_seeds, use_yarn):
    ...
    try:
        ...
        q_torch = torch.randn(1, batch_per_row, 32, hf_config.head_dim, dtype=torch.bfloat16)

        grid_size = ttnn.CoreCoord(8, 8)  # Safe limit: max 8 per dimension to avoid Galaxy hangs
        batch_grid = ttnn.num_cores_to_corerangeset(batch_per_row, grid_size, row_wise=True)

        q_mem_config = ttnn.create_sharded_memory_config(
            shape=(32, hf_config.head_dim),
            core_grid=batch_grid,
            strategy=ttnn.ShardStrategy.HEIGHT,
            orientation=ttnn.ShardOrientation.ROW_MAJOR,
            use_height_and_width_as_shard_shape=True,
