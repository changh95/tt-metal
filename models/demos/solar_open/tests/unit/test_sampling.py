...
"""
End-to-end on-device sampling tests for Solar-Open-100B on a Galaxy [4,8] mesh.

Tests TTSampling, TTPenalties, and LogProbsCalculator with the Solar-Open
vocab dimensions (196608, TP=8 -> 32768 padded per device) to verify:
...
from models.demos.solar_open.tests.test_factory import TRACE_MODEL_KEY, get_num_devices
from models.demos.solar_open.tt.model import compute_per_device_vocab
from models.demos.utils.trace_region_sizes import TRACE_MODEL_KEY_PARAM

# Every test in this module is pinned to a 4×8 Galaxy mesh with FABRIC_1D_RING
# (see SOLAR_OPEN_DEVICE_PARAMS below). On systems with fewer devices — the 1x8
# P150 LoudBox or a 1×1 P150 dev box — pytest's mesh_device fixture would TT_FATAL
# when trying to open a 4×8 mesh; skip the whole module instead so the rest of the
# unit-test suite still runs. The 1x8 on-device sampling path is covered end to end
# by the text demo and by test_modules.py::test_model.
pytestmark = pytest.mark.skipif(
    get_num_devices() < 32,
    reason="Solar-Open sampling tests require a 4×8 Galaxy mesh (32 devices)",
)
...
# Solar-Open device params: FABRIC_1D_RING + the trace region of the solar-open-100b YAML block.
# NOT Llama Galaxy's dispatch_core_axis/worker_l1_size/small trace.
SOLAR_OPEN_DEVICE_PARAMS = {
    "fabric_config": ttnn.FabricConfig.FABRIC_1D_RING,
    TRACE_MODEL_KEY_PARAM: TRACE_MODEL_KEY,
}


def make_solar_open_sampling_args(mesh_device, sampling_dp=1, use_topk_logprobs=False):
    """Create args matching the Solar-Open model on a Galaxy [4,8] mesh (Model._make_sampling_args).
...
def make_sharded_logits(torch_input, mesh_device, cluster_shape, dtype=ttnn.bfloat8_b):
    """Shard logits across cols (TP axis) for a [4,8] mesh.

    The model shards the vocab across mesh columns (axis 1). dims=(None, 3) means
...
@pytest.mark.parametrize("device_params", [SOLAR_OPEN_DEVICE_PARAMS], indirect=True)
def test_solar_open_greedy_sampling(sampling_params, batch_size, mesh_device, device_params, reset_seeds):
    """Test greedy (argmax) on-device sampling matches torch reference."""
    args = make_solar_open_sampling_args(mesh_device, sampling_dp=1)
    ...
    # Create random logits with the padded vocab dimensions
...
@pytest.mark.parametrize("mesh_device", [(4, 8)], indirect=True)
@pytest.mark.parametrize("device_params", [SOLAR_OPEN_DEVICE_PARAMS], indirect=True)
def test_solar_open_stochastic_sampling(sampling_params, batch_size, mesh_device, device_params, reset_seeds):
...
@pytest.mark.parametrize("mesh_device", [(4, 8)], indirect=True)
@pytest.mark.parametrize("device_params", [SOLAR_OPEN_DEVICE_PARAMS], indirect=True)
def test_solar_open_penalties(penalty_params, mesh_device, device_params, reset_seeds):
...
@pytest.mark.parametrize("mesh_device", [(4, 8)], indirect=True)
@pytest.mark.parametrize("device_params", [SOLAR_OPEN_DEVICE_PARAMS], indirect=True)
def test_solar_open_logprobs(mesh_device, device_params, reset_seeds):
    ...
    sg.reset_sampling_params(params)
    # See ``test_solar_open_stochastic_sampling`` — ``reset_sampling_params`` doesn't
...
@pytest.mark.parametrize("mesh_device", [(4, 8)], indirect=True)
@pytest.mark.parametrize("device_params", [SOLAR_OPEN_DEVICE_PARAMS], indirect=True)
def test_solar_open_topk_logprobs(mesh_device, device_params, reset_seeds):
    """Test top-K logprobs (new path) for Solar-Open-100B.
    ...
    args = make_solar_open_sampling_args(mesh_device, sampling_dp=1, use_topk_logprobs=True)
...
@pytest.mark.parametrize("mesh_device", [(4, 8)], indirect=True)
@pytest.mark.parametrize("device_params", [SOLAR_OPEN_DEVICE_PARAMS], indirect=True)
def test_solar_open_seed_determinism_batch_replay(mesh_device, device_params, reset_seeds):
...
@pytest.mark.parametrize("mesh_device", [(4, 8)], indirect=True)
@pytest.mark.parametrize("device_params", [SOLAR_OPEN_DEVICE_PARAMS], indirect=True)
def test_solar_open_seed_determinism_across_requests(mesh_device, device_params, reset_seeds):
