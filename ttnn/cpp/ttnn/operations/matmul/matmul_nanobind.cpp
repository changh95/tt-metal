...
void py_module(nb::module_& mod) {
    ...
    ttnn::bind_function<"sparse_matmul">(
        ...
        Keyword Args:
            ...
            expert_groups (int, optional): enables EXPERT-GROUP PARALLELISM (EGP). Defaults to ``None``, which runs the legacy kernels unchanged. With an integer ``G >= 1`` the op uses ``G x (number of output blocks)`` cores of ``compute_with_storage_grid_size`` (the first cores in row-major order, which must fill an exact rectangle): the non-zero sparsity slots are numbered by their running rank ``r`` in scan order and group ``g = r % G`` computes slot ``r`` (in indexed mode group ``i % G`` computes entry ``i``), so up to ``G`` experts stream their weights concurrently. Every core decides the validity of every slot locally, removing the per-slot multicast round trip, and a broadcast ``input_tensor_a`` (``[1, 1, M, K]`` with ``is_input_b_sparse`` only) is multicast once and kept resident in L1 for the whole op instead of being re-read per expert (its ``per_core_M x Kt`` tiles must fit L1 together with the double-buffered weight block). The per-output-tile math, the output shape and the output layout (expanded, compact or indexed) are unchanged, so the result is bit-identical to ``expert_groups=None``. Requirements: ``is_input_b_sparse`` must be True; ``input_tensor_a`` must have a single outer batch when only B is sparse; ``1 <= G <= E``; ``G x blocks >= 2``; with a broadcast A either ``per_core_M == out_block_h`` or ``per_core_N == out_block_w``. ``G == 1`` is a valid configuration (local validity and resident in0 on the legacy grid). With ``expert_groups`` set the kernels also write EVERY tile of the output themselves (the cores of group ``slot % G`` zero-fill each slot nobody computes with the same tile walk as a computed slot -- issued by the in0 reader kernel by default, or by the in1 writer with ``TT_SPARSE_MATMUL_EGP_ZERO_FILL=in1``; indexed and compact outputs are fully written by construction), so the op skips the zero-fill pass it runs before every legacy-path launch -- a pre-poisoned ``optional_output_tensor`` still comes back exactly equal to the legacy result. Requires ``Mt % per_core_M == 0`` and an INTERLEAVED output; keep the output in L1 (the zero tiles are NoC writes that would compete with the expert-weight DRAM reads on a DRAM output). ``TT_SPARSE_MATMUL_EGP_ZERO_FILL=0`` (per process) restores the host zero-fill pass for A/B measurements.
